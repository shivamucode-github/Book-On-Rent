<header class="flex gap-12 items-center justify-between border-b border-gray-500 bg-white px-10">
    <a href="{{ route('dashboard') }}" class="block w-28 h-28 outline-none">
        <x-application-logo />
    </a>
    <nav>
        <ul class="flex items-center gap-8">
            <li><a class="hover:text-blue-500" href="/">Home</a></li>
            <li class="relative" x-data="{ open: false }">
                <button x-on:click="open = ! open">Find</button>
                <ul x-show="open" class="absolute top-10 -left-2 z-10 bg-white border border-gray-300 divide-y divide-gray-400">
                    <li>
                        <x-category-dropdown />
                    </li>
                    <li>
                        <x-author-dropdown />
                    </li>
                </ul>
            </li>
            <li class="relative" x-data="{ open: false }">
                <a href="/return">Return Book</a>
            </li>
            <li class="relative" x-data="{ open: false }">
                <x-cart-component />
            </li>
        </ul>
    </nav>
    <div class="flex items-center gap-6">
        <form action="/books" method="get">
            @if (request('category'))
                <input type="hidden" name="category" value="{{ request('category') }}">
            @endif

            @if (request('author'))
                <input type="hidden" name="author" value="{{ request('author') }}">
            @endif

            <input type="search" value="{{ request('search') }}" name="search" id="search"
                placeholder="Search here.. "
                class="outline-none border-2 focus:border-blue-500  border-gray-300 py-2 px-6 rounded-full">
        </form>

        <div class="relative" x-data="{ open: false }">
            <button x-on:click="open = ! open">
                @php
                    if (Auth::user()->image) {
                        $image = asset('storage/' . Auth::user()->image);
                    } else {
                        $image =
                            'data:image/jpeg;base64,';
                    }
                @endphp
                <img src="{{ $image }}" alt="" class="w-14 h-14 rounded-full object-cover">
                <span class="flex items-center">
                    {{ Auth::user()->name }}
                    <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        <path fill="currentColor"
                            d="M13.098 8H6.902c-.751 0-1.172.754-.708 1.268L9.292 12.7c.36.399 1.055.399 1.416 0l3.098-3.433C14.27 8.754 13.849 8 13.098 8Z">
                        </path>
                    </svg>
                </span>
            </button>
            <div x-show="open" class="w-36 border border-gray-400 bg-gray-200 absolute top-18 right-0 z-10">
                <ul class="w-full text-sm space-y-2">
                    {{-- <li><a class="p-2 block hover:bg-gray-300" href="/profile">Edit Profile</a></li> --}}
                    <li><a class="p-2 block hover:bg-gray-300" href="/logout">Logout</a></li>
                </ul>
            </div>
        </div>
    </div>
</header>
<x-flash />
{{-- <header class="sm:hidden w-screen fixed inset-x-0 flex flex-col items-center justify-between bg-white z-40 gap-2">
    <div class="flex justify-between w-full py-8 px-4">
        <div class="flex items-center justify-center gap-3">
            <a href="#">
                <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                    <path
                        d="M19 11H1a1 1 0 010-2h18a1 1 0 110 2zm0-7H1a1 1 0 010-2h18a1 1 0 110 2zm0 14H1a1 1 0 010-2h18a1 1 0 010 2z"
                        fill="currentColor"></path>
                </svg>
            </a>
            <div class="flex items-center justify-center">
                <a href="{{ route('dashboard') }}" class="block w-24 h-24 outline-none">
                    <x-application-logo />
                </a>
            </div>
        </div>
        <a href="#"
            class="flex items-center gap-3 px-4 text-xs font-semibold py-1 border border-black rounded-full">
            <span>
                <svg class="w-8 h-8 text-yellow-500" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                    stroke="none" viewBox="0 0 24 24">
                    <path
                        d="M21.822 7.431A1 1 0 0 0 21 7H7.333L6.179 4.23A1.994 1.994 0 0 0 4.333 3H2v2h2.333l4.744 11.385A1 1 0 0 0 10 17h8c.417 0 .79-.259.937-.648l3-8a1 1 0 0 0-.115-.921z">
                    </path>
                    <circle cx="10.5" cy="19.5" r="1.5"></circle>
                    <circle cx="17.5" cy="19.5" r="1.5"></circle>
                </svg>
            </span>
            <span>Cart</span>
            <svg class="w-4 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                aria-hidden="true">
                <path fill-rule="evenodd"
                    d="M3.75 12a.75.75 0 01.75-.75h13.19l-5.47-5.47a.75.75 0 011.06-1.06l6.75 6.75a.75.75 0 010 1.06l-6.75 6.75a.75.75 0 11-1.06-1.06l5.47-5.47H4.5a.75.75 0 01-.75-.75z"
                    clip-rule="evenodd"></path>
            </svg>
        </a>
    </div>
    <nav class="w-full">
        <ul class="flex flex-col items-center text-sm font-semibold divide-y divide-gray-500">
            <li class="block w-full py-4 px-4">
                <select name="category" id="category">
                    <option value="">Category</option>
                    <option value="second">any category</option>
                </select>
            </li>
            <li class="block w-full py-4 px-4">
                <select name="about" id="about">
                    <option value="">Author</option>
                    <option value="policy">rules & policy</option>
                </select>
            </li>
            <li class="block w-full py-4 px-4">
                <input type="search" value="" name="search" id="search" placeholder="Search here.. "
                    class="outline-none border focus:border-blue-500  border-gray-300 py-2 px-6 rounded-full">
            </li>
        </ul>
    </nav>
</header>

<header class="hidden fixed inset-x-0 sm:flex items-center justify-between py-5 px-28 bg-white w-screen z-40 gap-8">
    <div class="flex items-center justify-center">
        <a href="{{ route('dashboard') }}" class="block w-24 h-24 outline-none">
            <x-application-logo />
        </a>
    </div>
    <nav>
        <ul class="flex items-center gap-1 text-sm font-semibold">
            <li>
                <select name="author" id="author" class="border-none outline-none">
                    <option value="">Author</option>
                    <option value="second">any category</option>
                </select>
            </li>
            <li>
                <select name="category" id="category" class="border-none outline-none">
                    <option value="">Category</option>
                    <option value="policy">rules & policy</option>
                </select>
            </li>
            <li>
                <input type="search" value="" name="search" id="search" placeholder="Search here.. "
                    class="outline-none border focus:border-blue-500  border-gray-300 py-2 px-6 rounded-full">
            </li>
        </ul>
    </nav>
    <a href="#" class="flex items-center gap-3 px-4 text-md font-semibold py-1 border border-black rounded-full">
        <span><svg class="w-8 h-8 text-yellow-500" xmlns="http://www.w3.org/2000/svg" fill="currentColor" stroke="none"
                viewBox="0 0 24 24">
                <path
                    d="M21.822 7.431A1 1 0 0 0 21 7H7.333L6.179 4.23A1.994 1.994 0 0 0 4.333 3H2v2h2.333l4.744 11.385A1 1 0 0 0 10 17h8c.417 0 .79-.259.937-.648l3-8a1 1 0 0 0-.115-.921z">
                </path>
                <circle cx="10.5" cy="19.5" r="1.5"></circle>
                <circle cx="17.5" cy="19.5" r="1.5"></circle>
            </svg></span>
        <span>Cart</span>
        <svg class="w-4 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
            aria-hidden="true">
            <path fill-rule="evenodd"
                d="M3.75 12a.75.75 0 01.75-.75h13.19l-5.47-5.47a.75.75 0 011.06-1.06l6.75 6.75a.75.75 0 010 1.06l-6.75 6.75a.75.75 0 11-1.06-1.06l5.47-5.47H4.5a.75.75 0 01-.75-.75z"
                clip-rule="evenodd"></path>
        </svg>
    </a>
</header> --}}
